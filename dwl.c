...
void buttonpress(struct wl_listener *listener, void *data) {
  ...
  case WL_POINTER_BUTTON_STATE_PRESSED:
    ...
    if (!c && !exclusive_focus &&
      ...
      cx = (cursor->x - selmon->m.x - borderpx - sidepad) *
...
void motionabsolute(struct wl_listener *listener, void *data) {
   ...
   * window from any edge, so we have to warp the mouse there. There is also
   * some hardware which emits these events. */
...
void motionnotify(uint32_t time, struct wlr_input_device *device, double dx,
  ...
  /* If there's no client surface under the cursor, set the cursor image to
   * a default. This is what makes the cursor image appear when you move it
...
void rendermon(struct wl_listener *listener, void *data) {
   ...
   * the rest of the state in one go, but to do that we would need to
   * rewrite wlr_scene_output_commit() in order to add the gamma to the
   * pending state before committing, instead try to commit the gamma in one
   * frame, and commit the rest of the state in the next one (or in the same
   * frame if the gamma can not be committed).
   */
...
void setup(void) {
   ...
   * but we need to create manually the linux_dmabuf interface to integrate
   * it with wlr_scene. */
